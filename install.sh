...
if [ -n "$BROWSER" ]; then
  ...
  warn "The app will still install; run 'filefinder --serve' and open the printed"
...
sed -e "s|@EXEC@|$BIN_DIR/filefinder|g" "$SRC/desktop/filefinder.desktop.in" \
  > "$DESKTOP_DIR/filefinder.desktop"
chmod 0644 "$DESKTOP_DIR/filefinder.desktop"
...
if [ -d "$DESK" ]; then
  cp "$DESKTOP_DIR/filefinder.desktop" "$DESK/File Finder.desktop"
  chmod +x "$DESK/File Finder.desktop"
  gio set "$DESK/File Finder.desktop" metadata::trusted true 2>/dev/null || true
  echo "    shortcut placed on $DESK"
fi

# ---------- first index ----------
say "Indexing your files (this scans \$HOME)"
python3 "$APP_DIR/index_files.py" || warn "Indexing failed — run 'filefinder --reindex' later."
...
cat <<DONE
  ...
  File Finder installed.

    Launch:      filefinder
    Or:          your app menu -> "File Finder"
    Re-scan:     filefinder --reindex
    No browser:  filefinder --serve   (then open the printed URL)
    Remove:      ./uninstall.sh
